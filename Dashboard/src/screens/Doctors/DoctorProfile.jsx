...
function DoctorProfile() {
  ...
  // Fetch doctor data from location or API
  useEffect(() => {
    if (location.state?.doctorData) {
      console.log('Using doctor data from location:', location.state.doctorData);
        ...
        return (
          doctorData && (
            <DoctorInfo
              ...
              onSave={(updatedData) => {
                console.log('Doctor info updated:', updatedData);
    ...
    <Layout>
      <div className="flex items-center gap-4">
        ...
        <h1 className="text-xl font-semibold">
          {doctorData ? `Dr. ${doctorData.fullName}` : 'Doctor Profile'}
        </h1>
      </div>
        ...
        <div className="col-span-12 lg:col-span-4 bg-white rounded-xl border-[1px] border-border p-6 lg:sticky top-28 flex items-center justify-center flex-col">
          ...
          <div className="gap-2 flex-col">
            ...
            <p className="text-xs text-textGray">
              {doctorData ? doctorData.email : 'Loading...'}
            </p>
            <p className="text-xs">
              {doctorData ? doctorData.phone : 'Loading...'}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-3 px-2 xl:px-12 w-full">
            {doctorTab.map((tab, index) => (
              <button
                ...
                onClick={() => {
                  setActiveTab(tab.id);
                  console.log(`Switched to tab: ${tab.title} (Tab ID: ${tab.id})`);
                }}
              >
