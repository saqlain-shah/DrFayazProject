...
function Campaings() {
  ...
  const shareViaEmail = async (campaign) => {
    try {
      ...
      const res = await axios.get('http://localhost:8800/api/patients/', {
        headers: { Authorization: `Bearer ${token}` }
      });

      // Filter patients to get unique emails
      const newEmailContacts = res.data
        .filter(patient => patient.email && !contacts.some(contact => contact.email === patient.email))
        .map(patient => ({ name: patient.fullName, email: patient.email }));

      setContacts(prevContacts => [...prevContacts, ...newEmailContacts]);
      setIsDialogOpen(true);
      setShowShareOptions(false);
  ...
  const shareViaWhatsApp = async (campaign) => {
    try {
      ...
      // Filter patients to get unique phone numbers
      const newWhatsAppContacts = res.data
        .filter(patient => patient.emergencyContact && !contacts.some(contact => contact.phoneNumber === patient.emergencyContact))
        .map(patient => ({ name: patient.fullName, phoneNumber: patient.emergencyContact }));

      setContacts(prevContacts => [...prevContacts, ...newWhatsAppContacts]);
      setIsDialogOpen(true);
      setShowShareOptions(false);
      setMessage(`Title: ${campaign.title}\nSend To: WhatsApp\nMessage: ${campaign.message}`);
    } catch (error) {
