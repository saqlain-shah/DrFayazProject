...
function NewMedicalRecord() {
  ...
  const addMedicineDosage = (medicineDosage) => {
    ...
    setMedicineDosages((prevMedicineDosages) => [
      ...prevMedicineDosages,
      medicineDosage,
    ]);
  ...
  const onChangeTreatments = (name, checked) => {
    const updatedTreatments = treatments.map(
      (item) => (item.name === name ? { ...item, checked: !!checked } : item) // Convert checked to Boolean
    );
        ...
        <div className="col-span-12 flex-colo gap-6 lg:col-span-4 bg-white rounded-xl border-[1px] border-border p-6 lg:sticky top-28">
          ...
          <div className="gap-2 flex-colo">
            <h2 className="text-sm font-semibold">
              {webPatientData?.patientInfo?.name || patientData?.fullName}
            </h2>
            <p className="text-xs text-textGray">
              {webPatientData?.patientInfo?.email || patientData?.email}
            </p>
            <p className="text-xs">
              ...
              <div className="grid xs:grid-cols-2 md:grid-cols-3 gap-6 pb-6">
                {servicesData?.slice(1, 100).map((item) => (
                  <Checkboxe
                    ...
                    checked={
                      (
                        treatments.find((i) => i.name === item.name) || {
                          checked: false,
                        }
                      ).checked
                    }
                    onChange={(checked) =>
                      onChangeTreatments(item.name, checked)
                    } // Call onChangeTreatments with the treatment name and checked value
                    name={item.name}
                    key={item.id}
                  />
                ))}
