...
const DoctorInvoice = () => {
    ...
    const dummyData = [
        {
            doctor: { appointment: { patient: { firstName: 'Sajjad', lastName: 'Hussain' } } },
            totalAmount: 300,
            createdAt: '2023-02-09T12:00:00Z',
            paymentMethod: 'easypaisa',
            paymentType: 'Online',
            appointmentId: '123435'
        },
        {
            doctor: { appointment: { patient: { firstName: 'John', lastName: 'Doe' } } },
            totalAmount: 100,
    ...
    const columns = [
        {
            ...
            render: (data) => (
                <div className="table-avatar">
                    <a className="avatar avatar-sm mr-2 d-flex gap-2">
                        <img className="avatar-img rounded-circle" src={img} alt="" />
                        <p className='p-0 m-0 text-nowrap'>{`${data.doctor.appointment.patient.firstName} ${data.doctor.appointment.patient.lastName}`}</p>
                    </a>
                </div>
            )
        },
        {
            ...
            key: '2',
            width: 100,
            render: (data) => <div>{data.totalAmount} $</div>
        },
        {
            title: 'Paid On',
            key: '3',
            width: 100,
            render: (data) => <div>{moment(data.createdAt).format("LL")}</div>
        },
        {
            title: <div className='text-nowrap'>Payment Method</div>,
            key: '4',
